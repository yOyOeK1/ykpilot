...
class driver5:
	...
	def get_discrete_state(self,b):
		...
		n = "%s_%s_%s"%(cefoldIn,grfoldIn,tola)
	...
	def mainLoop(self):
		...
		while self.itersLeft > 0:
			...
			if m.fabs(b['cogError'])<0.0 and m.fabs(b['cogError']) < 10.00:
				reward = -0.1-((m.fabs(b['cogError'])/45.00)*0.8)
			elif b['cogError'] == 0.0:
				reward = -0.05
			
			if iterNo>2:
				if b['cogError'] == 0.0 and self.timeOfOff>1.0:
					reward = 0.0
